...
class IK:
    ...
    def distance_and_angle(G, H):
        ...
        angle = acos(np.clip((np.trace(G[:3,:3] @ H[:3,:3].T) - 1)/2., -1, 1))
    ...
    def is_valid_solution(self,q,target):
        ...
        cur_trans = self.fk.forward(q)[1]
        dist, ang = self.distance_and_angle(target, cur_trans)
        if dist <= self.linear_tol and ang <= self.angular_tol and np.all(q >= self.lower) and np.all(q <= self.upper):
            success = True
            message = "Solution found.\n"
        else:
            message = "Solution not found.\n"
            if dist > self.linear_tol:
                message += " Out of the position tolerant: dist: " + str(dist) + " linear_tol: " + str(self.linear_tol) + "\n"
            if ang > self.angular_tol:
                message += " Out of the angular tolerant: ang: " + str(ang) + " angular_tol: " + str(self.angular_tol) + "\n"
            if not np.all(q >= self.lower) or not np.all(q <= self.upper):
                message += " Out of the joint limits: q: " + str(q) + " lower: " + str(self.lower) + " upper: " + str(self.upper) + "\n"
    ...
    def end_effector_task(q,target,method):
        ...
        dq = np.zeros(7)
        v = np.zeros(6)
        v[:3] = IK.displacement_and_axis(target, IK.fk.forward(q)[1])[0]
        v[3:] = IK.displacement_and_axis(target, IK.fk.forward(q)[1])[1]
        jac = calcJacobian(q)
        if method == 'J_pseudo':
    ...
    def inverse(self, target, seed, method, alpha):
        ...
        while True:
            ...
            dq = dq_ik + (np.eye(7) - jac_pinv @ jac) @ dq_center

            # Check termination conditions
            if len(rollout) > self.max_steps or np.linalg.norm(dq) < self.min_step_size:
            # if len(rollout) > self.max_steps or np.linalg.norm(dq) < self.min_step_size or (q < self.lower).any() or (q > self.upper).any():
                break
