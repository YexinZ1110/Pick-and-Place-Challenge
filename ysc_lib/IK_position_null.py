...
class IK:
    ...
    def distance_and_angle(G, H):
        ...
        R = G[:3, :3] @ H[:3, :3].T
        trace = np.trace(R)
        angle = acos(np.clip((trace - 1.0) / 2.0, -1.0, 1.0))
        ## END STUDENT CODE
    ...
    def is_valid_solution(self,q,target):
        ...
        T0e = self.fk.forward(q)[1]
        d, ang = self.distance_and_angle(target, T0e)
    ...
    def end_effector_task(q,target,method):
        ...
        if method == 'J_pseudo':
            dq = np.linalg.pinv(J) @ np.concatenate((displacement, axis))
        else:
    ...
    def inverse(self, target, seed, method, alpha):
        ...
        while True:
            ...
            ## Task Prioritization
            J = calcJacobian(q)
            N = ((np.eye(7) - np.linalg.pinv(J) @ J) @ dq_center).T
            dq = dq_ik + N

            # Check termination conditions
            if self.is_valid_solution(q,target)[0] or i >= self.max_steps:
                break
